...
- To run the whole training/evaluation pipeline: run `main.py`. This script is prepared to do the followings:
    ...
    - Initializes the neural network as defined in the `improved_net_128.py` file.
    ...
    - During multiple training sessions you can change `filename` so that the names of your files are easy to comprehend
    ...
    - Save your trained model's weights in the `/model_weights/` subdirectory so that you can reload them later.
- To test the data: run the `model_evaluation.py` file to get the confusion matrices and ROC curves
  - Make sure to run this with the same `model`, `batchsize` and `filename` as you did in `main.py` 
  - For checking the code, if there are no alterations made to these variables it will run without an issue.  
